...
def getStoryRuntime(botHash, platform, userId):
    res = OlivaDiceCore.userConfig.getUserConfigByKey(
        userId=userId, userType='user', platform=platform, userConfigKey='storyRuntime', botHash=botHash
    )
...
def setStoryRuntime(botHash, platform, userId, storyRuntime):
...
def getStory(botHash, storyName):
    res = None
    if (
        botHash in OlivaStoryCore.storyEngine.storyList
...
def getStoryNodeByFlagForData(storyData, flag):
    res = None
    story = storyData
    if story is not None:
        for storyNode in story['story']:
            if (
                type(storyNode) is dict
...
def getStoryNodeByFlag(storyName, flag, botHash):
    res = getStoryNodeByFlagForData(storyData=getStory(botHash=botHash, storyName=storyName), flag=flag)
    return res
...
def getStoryNodeData(storyName: str, flag: str, dataKey: str, botHash: str):
    res = getStoryNodeDataForStoryData(
        storyData=getStoryNodeByFlag(storyName=storyName, flag=flag, botHash=botHash), dataKey=dataKey
    )
    return res
...
def setStoryNoteList(listData: list, nodeData: dict):
    ...
    if type(listData) is not list:
        res = []
    if type(nodeData) is dict and 'hideNote' in nodeData and type(nodeData['hideNote']) is list:
        for hideNote_item in nodeData['hideNote']:
            if type(hideNote_item) is str and hideNote_item not in res:
                res.append(hideNote_item)
    return res


def haveStoryNote(selectionData: dict, storyNoteList: list):
    res = False
    if (
        type(selectionData) is dict
        ...
        res = True
    elif type(selectionData) is dict and 'hideNote' not in selectionData:
        res = True
    return res


def startStory(botHash: str, platform: str, userId: str, storyName: str, chatToken: str):
    res = None
    story = getStory(botHash, storyName)
    if story is not None:
        tmp_story_runtime = getStoryRuntime(botHash=botHash, platform=platform, userId=userId)
        tmp_storyNode = getStoryNodeByFlagForData(storyData=story, flag=story['ingress'])
        res = tmp_storyNode
        if res is not None:
            ...
            setStoryRuntime(botHash=botHash, platform=platform, userId=userId, storyRuntime=tmp_story_runtime)
    return res


def runStoryBySelectionIndex(
    botHash: str, platform: str, userId: str, chatToken: str, selectionIndex: 'None|str|int' = None
):
    res = None
    tmp_story_runtime = getStoryRuntime(botHash=botHash, platform=platform, userId=userId)
    storyFlagNext = None
    flagNeedSave = False
    if (
        chatToken in tmp_story_runtime
        ...
        if story is not None:
            tmp_storyNode = getStoryNodeByFlagForData(
                storyData=story, flag=tmp_story_runtime[chatToken]['storyFlagNow']
            )
            tmp_story_runtime[chatToken]['storyNoteList'] = setStoryNoteList(
                tmp_story_runtime[chatToken].get('storyNoteList', []), tmp_storyNode
            )
            ...
            if tmp_storyNode_type == None and type(selectionIndex) is int:
                ...
                tmp_storyNode_raSkillValue = getStoryNodeDataForStoryData(
                    dataKey='raSkillValue', storyData=tmp_storyNode
                )
                tmp_storyNode_raMap = getStoryNodeDataForStoryData(dataKey='raMap', storyData=tmp_storyNode)
                tmp_userData_raRollValue = OlivaDiceCore.userConfig.getUserConfigByKey(
                    userConfigKey='RDRecordInt', botHash=botHash, userId=userId, userType='user', platform=platform
                )
                tmp_userData_raSkillValue = OlivaDiceCore.userConfig.getUserConfigByKey(
                    userConfigKey='RDRecordSkillInt', botHash=botHash, userId=userId, userType='user', platform=platform
                )
                if (
                    tmp_storyNode_raSkillName is not None
                    and type(tmp_userData_raRollValue) is int
                    and type(tmp_userData_raSkillValue) is int
                    and (
                        (
                            type(tmp_storyNode_raSkillValue) is int
                    ...
                    for tmp_storyNode_raMap_item in tmp_storyNode_raMap:
                        if (
                            type(tmp_storyNode_raMap_item) is dict
            ...
            if type(selectionIndex_new) is int:
                if (
                    len(tmp_storyNode_selection) > selectionIndex_new
                    and 0 <= selectionIndex_new
                    and type(tmp_storyNode_selection[selectionIndex_new]) is dict
                ):
                    if (
                        'to' in tmp_storyNode_selection[selectionIndex_new]
                        ...
                        if haveStoryNote(
                            ...
                            res = getStoryNodeByFlagForData(
                                storyData=story, flag=tmp_storyNode_selection[selectionIndex_new]['to']
                            )
                            if res is not None:
